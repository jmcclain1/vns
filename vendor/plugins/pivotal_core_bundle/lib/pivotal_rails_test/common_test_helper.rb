...
module CommonTestHelper
  ...
  def assert_email_sent(expected_from, expected_to, expected_subject, expected_body = nil)
    ...
    assert_equal expected_subject, delivery.subject
  ...
  def assert_array_contents_match(collection_to_match, collection_to_test, message = "")
    assert_equal collection_to_match.length, collection_to_test.length, message
    assert((collection_to_match - collection_to_test).empty?, message)
  end
