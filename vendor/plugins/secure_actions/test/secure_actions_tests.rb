...
class SecureActionsController < ActionController::Base
  ...
  require_ssl :foo, :bar, :xhr_ssl_action
  xhr_actions :xhr_action, :xhr_ssl_action
  ...
  def xhr_action
    render :text => "non ssl xhr action"
  end

  def xhr_ssl_action
    render :text => "ssl xhr action"
  end
...
class SecureActionsTest < Test::Unit::TestCase
  ...
  def test_ssl_url_relative_only
    get :index
    assert_equal "/secure_actions/xhr_action",  @controller.url_for(:action => "xhr_action", :only_path => true)
  end
